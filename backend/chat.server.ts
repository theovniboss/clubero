...
const server = Bun.serve<WebSocketData>({ 
    ...
    async fetch(req, server) {
        ...
        const isAuthorized = await chatController.canUserJoinChatRoom(userId, clubId, teamId);
        if (!isAuthorized) {
...
console.log(`WebSocket server listening on ws://${server.hostname}:${server.port}`);